...
router.get('/cq2', function(req, res) {
	...
	var url = 'http://d3.poojit.com:8080/solr/oodt-fm/select?q=postedDate:['+startDate+'-01T00:00:00Z%20TO%20'+endDate+'-31T00:00:00Z]%20company:'+company_name+'&facet=true&facet.pivot=geohash4,geohash1&f.geohash4.facet.limit=100&f.geohash1.facet.limit=1&wt=json&indent=false';
	console.log('Solr URL: ' + url);
	request(url, function (error, response, body) {
	  if (!error && response.statusCode == 200) {
	  	body = JSON.parse(body);
...
router.get('/cq4', function(req, res) {
	...
	var url = 'http://d3.poojit.com:8080/solr/oodt-fm/select?q=computer%20postedDate:['+startDate+'-01T00:00:00Z%20TO%20'+endDate+'-31T00:00:00Z]&facet=true&facet.pivot=geohash3,geohash1&f.geohash3.facet.limit=500&f.geohash1.facet.limit=1&wt=json&indent=false';
